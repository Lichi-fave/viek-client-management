...
### 5. App crashed before first load
...
### 6. Adding a client silently failed
...
### 7. New client IDs could collide after a delete

**Where:** `server.js`, `POST /api/clients`
**Root cause:** `id: clients.length + 1` assumes the array only ever grows. Once deletes actually started working (see #2), this could reassign an ID still in use.
**Fix:** A `nextClientId` counter that only increments, never reused.

### 8. Plaintext passwords and a single shared fake token
